...
function telephoneCheck(str) {
    let regNormalThinking = new RegExp(/^(1)?(\()?(\d{3}[-\)]?){3}\d{1,4}$/, 'g');
    let isLengthOf10 = str.replace(/\s|\D/g, '').replace(/^1/g, '').length == 10;
    if (isLengthOf10) {
        let leftParentheses = [...str].filter(ele => ele == '(');
        let rightParentheses = [...str].filter(ele => ele == ')');
        const regParentheses = new RegExp(/\(|\)/, 'g');
        if (!regParentheses.test(str)) {
            return regNormalThinking.test(str.replace(/\s/g, ''));
        } else {
